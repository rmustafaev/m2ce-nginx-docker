...
sed -i "s|DOMAIN_NAME|${DOMAIN_NAME}|" /etc/nginx/conf.d/mage.conf
...
#/usr/local/bin/acme-nginx -d ${DOMAIN_NAME}

#cat <<EOF | crontab -u root -
#30 2 15 * * /usr/local/bin/acme-nginx -d ${DOMAIN_NAME} && service nginx reload >> /var/log/letsencrypt.log 2>&1
#EOF

#else 
#  echo -e "No need to generate and use SSL certificate for ${DOMAIN_NAME}"
#fi
